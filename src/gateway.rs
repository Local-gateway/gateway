...
use crate::cache::{CacheMetadata, GatewayCache};
use crate::compression::{CompressionConfig, CompressionManager};
use crate::network::{NetworkEvent, NetworkManager};
use crate::performance::{BenchmarkResult, PerformanceMonitor, PerformanceTestSuite};
use crate::protocol::WdicMessage;
use crate::registry::{Registry, RegistryEntry};
use crate::tls::{MtlsConfig, TlsManager};
use crate::udp_protocol::{UdpBroadcastEvent, UdpBroadcastManager, UdpToken};
...
impl Gateway {
    ...
    ///
    /// 解压缩后的文件数据和元数据（如果存在）
    pub async fn get_cached_file_by_name(
        &self,
        name: &str,
    ) -> Result<Option<(Vec<u8>, CacheMetadata)>> {
        let mut cache = self.cache.lock().await;
    ...
    ///
    /// 包含所有测试结果的映射表
    pub async fn run_comprehensive_performance_tests(
        &self,
    ) -> Result<ahash::AHashMap<String, BenchmarkResult>> {
        info!("开始运行综合性能测试套件");
        ...
        match self
            .performance_monitor
            .run_throughput_benchmark("network_throughput", &throughput_suite)
            .await
        {
            Ok(result) => {
        ...
        match self
            .performance_monitor
            .run_throughput_benchmark("udp_broadcast", &udp_suite)
            .await
        {
            Ok(result) => {
        ...
        match self
            .performance_monitor
            .run_latency_benchmark("message_latency", 1000)
            .await
        {
            Ok(result) => {
    ...
    async fn test_memory_usage(&self) -> Result<BenchmarkResult> {
        ...
        parameters.insert(
            ...
            format!("{:.2}", peak_memory as f64 / 1024.0 / 1024.0),
        );
            ...
            let handle = tokio::spawn(async move {
                for i in 0..operations_per_task {
                    ...
                    let entry = RegistryEntry::new(
                        ...
                        std::net::SocketAddr::from((
                            [172, 16, (task_id % 255) as u8, (i % 255) as u8],
                            55555 + i as u16,
                        )),
                    );
    ...
    pub async fn quick_performance_check(&self) -> Result<QuickPerformanceStats> {
        ...
        Ok(QuickPerformanceStats {
            ...
            uptime_seconds: chrono::Utc::now().timestamp()
                - self.get_local_entry().await.last_seen.timestamp(),
        })
    }
}
